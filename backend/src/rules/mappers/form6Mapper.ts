...
export function buildForm6Data(
  ...
  const workerZip = splitPostalCode(
    rawInput["本人郵便番号(例: 123-4567)"] || rawInput["本人郵便番号"] || ""
  );
  const compZip = splitPostalCode(
    rawInput["会社郵便番号"] || rawInput["事業場郵便番号"] || ""
  );
  const mainHospZip = splitPostalCode(
    rawInput["病院郵便番号"] || rawInput["診療受けた病院の郵便番号"] || ""
  );

  const workerPhone = splitPhoneNumber(
    rawInput["本人電話番号(例: 090-1234-5678)"] || rawInput["本人電話番号"] || ""
  );
  const compPhone = splitPhoneNumber(
    rawInput["会社電話番号"] || rawInput["事業場電話番号"] || ""
  );

  const birthYmd = parseYYMMDD(
    rawInput["生年月日(例: 55年5月15日→550515)"] || rawInput["生年月日"] || ""
  );
  const injuryYmd = parseYYMMDD(
    rawInput["負傷年月日(例: 令和8年8月29日→080829)"] || rawInput["負傷年月日"] || ""
  );
  ...
  const gender = convertGender(
    rawInput["性別(男性は1、女性は3)"] || rawInput["性別"] || ""
  );

  const rawTimeType = rawInput["負傷時刻区分(AM または PM)"] || rawInput["負傷時刻区分"] || "";
  const rawHour = rawInput["負傷時刻(時)"] || rawInput["負傷時刻時"] || "";
  const rawMin = rawInput["負傷時刻(分)"] || rawInput["負傷時刻分"] || "";
  const timeInfo = parseInjuryTime(rawTimeType, rawHour, rawMin);

  const laborIns = parseLaborInsurance(
    rawInput["特別加入の労働保険番号"] || rawInput["労働保険番号"] || ""
  );

  const prefStr = rawInput["住所都道府県"] || rawInput["都道府県"] || "";
  const cityStr = rawInput["住所市町村以降"] || rawInput["市区町村以降"] || "";
  const fullAddress = prefStr + cityStr;
  const workerName = rawInput["氏名(漢字)"] || rawInput["氏名"] || rawInput["労働者氏名"] || "";

  // 会社情報
  const compName = rawInput["会社名"] || rawInput["事業場名称"] || "";
  const compAddr = rawInput["会社住所"] || rawInput["事業場所在地"] || "";
  const repName = rawInput["代表者名"] || rawInput["事業主氏名"] || "";
