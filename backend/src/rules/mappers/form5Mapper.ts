...
export function buildForm5Data(
  ...
  // 5. 各種日付の分解 (通常項目は .year 等を使用, マス目項目は .padded6 を使用)
  ...
  return {
    ...
    "date_of_birth": birthYmd.padded6, // マス目印字（6桁）
    "Date_of_injury,Japanese_era": "9",
    "Date_of_injury": injuryYmd.padded6, // マス目印字（6桁）
    ...
    "Claim_Hospital_name": wrapText(cleanClaimHospName, 7), // カット済みの名称を印字
    ...
    "Year_of_proof_of_fact": proofDateYmd.year, // ゼロ埋めなし（例: 8）
    ...
    "Claimant's_address": fullAddress,     // 都道府県＋市町村を結合
    ...
    "Year_of_entry": fillDateYmd.year, // ゼロ埋めなし
    ...
    "Year_of_joining": joiningDateYmd.year, // ゼロ埋めなし
